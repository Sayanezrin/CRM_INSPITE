...
function PayslipManagerPage({ store, commit, createdBy }) {
  ...
  const [form, setForm] = useState(() => ({ employeeId: employees[0]?.id || "", month: today().slice(0, 7), basic: employees[0]?.salary || "", allowances: "", allowanceNotes: "", deductions: "", notes: "" }));
  ...
  const save = (event) => {
    ...
    const payslip = { id: editingId || uid("PAY"), employeeId: selectedEmployee.id, employeeName: selectedEmployee.name, employeeRole: selectedEmployee.role, department: selectedEmployee.department, month: form.month, basic: Number(form.basic || 0), allowances: Number(form.allowances || 0), allowanceNotes: String(form.allowanceNotes || "").trim(), deductions: Number(form.deductions || 0), notes: String(form.notes || "").trim(), createdBy, updatedAt: new Date().toISOString() };
    commit((current) => ({ ...current, payslips: [payslip, ...(current.payslips || []).filter((item) => item.id !== payslip.id)] }));
    setEditingId(null);
    setForm({ employeeId: employees[0]?.id || "", month: today().slice(0, 7), basic: employees[0]?.salary || "", allowances: "", allowanceNotes: "", deductions: "", notes: "" });
    toast("Payslip saved.");
  };
  const edit = (payslip) => { setEditingId(payslip.id); setForm({ employeeId: payslip.employeeId, month: payslip.month, basic: payslip.basic, allowances: payslip.allowances, allowanceNotes: payslip.allowanceNotes || "", deductions: payslip.deductions, notes: payslip.notes || "" }); window.scrollTo({ top: 0, behavior: "smooth" }); };
